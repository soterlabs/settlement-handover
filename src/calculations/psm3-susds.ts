...
export async function calculatePsm3SusdsProfit(
  ctx: DebtPnLContext,
): Promise<Psm3SusdsProfitResult> {
  const addrs: Array<{ chain: Chain; address: string }> = [];
  if (ctx.chains.includes("ethereum"))
    addrs.push({ chain: "ethereum", address: "0xa3931d71877c0e7a3148cb7eb4463524fec27fbd" });
  ...
  const bals = await fetchPsm3SusdsBalances(ctx, addrs);
...
async function fetchPsm3SusdsBalances(
  ctx: DebtPnLContext,
